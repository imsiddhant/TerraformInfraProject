...
resource "aws_launch_template" "LT_main" {
  ...
  user_data = filebase64(templatefile("${path.module}/app.sh", { DB_HOST = var.DB_HOST, DB_USER = var.DB_USER, DB_PASSWORD_PARAM = var.DB_PASSWORD_PARAM, DB_PORT = var.DB_PORT, DB_NAME= var.DB_NAME }))
